 ...
 * @since 1.0.0
 */
abstract class Billplz_FluentCart_Client {
     ...
     * @throws \Exception
     */
    protected function get( $route, $params = [] ) {
     ...
     * @throws \Exception
     */
    protected function post( $route, $params = [] ) {
    ...
    protected function request( $route, $params = [], $method = 'POST' ) {
        if ( !$this->api_key ) {
            throw new Exception( __( 'Missing API key', 'billplz-for-fluent-cart' ) );
        }
    ...
    public function get_ipn_response() {
        if ( !in_array( $_SERVER['REQUEST_METHOD'], [ 'GET', 'POST' ] ) ) {
            throw new Exception( __( 'Invalid IPN response', 'billplz-for-fluent-cart' ) );
        }
        ...
        foreach ( $params as $param ) {
            ...
            if ( !isset( $_POST[ $param ] ) ) {
                throw new Exception(
                    ...
                    sprintf( __( 'Missing IPN parameter - %s', 'billplz-for-fluent-cart' ), $param )
                );
            }

            $allowed_params[ $param ] = sanitize_text_field( stripslashes( $_POST[ $param ] ) );
        }
    ...
    private function get_valid_ipn_redirect_response() {
        ...
        foreach ( $params as $param ) {
            ...
            if ( !isset( $_GET['billplz'][ $param ] ) ) {
                /* translators: %s: IPN parameter */
                throw new Exception( sprintf( __( 'Missing IPN parameter - %s', 'billplz-for-fluent-cart' ), $param ) );
            }
            ...
            $allowed_params[ $param_new_key ] = sanitize_text_field( stripslashes( $_GET['billplz'][ $param ] ) );
        }
    ...
    public function validate_ipn_response( $response ) {
        if ( !$this->verify_signature( $response ) ) {
            throw new Exception( __( 'Signature mismatch', 'billplz-for-fluent-cart' ) );
        }
    ...
    private function verify_signature( $response ) {
        ...
        if ( !$ipn_signature ) {
            throw new Exception( __( 'Missing IPN signature', 'billplz-for-fluent-cart' ) );
        }
